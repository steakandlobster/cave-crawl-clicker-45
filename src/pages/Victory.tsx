        ...
        <div className="space-y-4 mb-8">
          <div className="bg-green-500/10 p-4 rounded-lg border border-green-500/20">
            ...
            <div className="bg-secondary/50 p-4 rounded-lg border border-border">
              ...
              <p className="text-2xl font-bold text-treasure-gold">{(state.initialCredits + netGain).toFixed((state.initialCredits + netGain) !== 0 && Math.abs(state.initialCredits + netGain) < 0.00001 ? 5 : (state.initialCredits + netGain) < 0.01 ? 4 : 3)} ETH</p>
            </div>
            ...
            <div className={`p-4 rounded-lg border ${isProfit ? 'bg-green-500/20 border-green-500/30' : 'bg-yellow-500/20 border-yellow-500/30'}`}>
              ...
              <p className={`text-xl font-bold ${isProfit ? 'text-green-400' : 'text-yellow-400'}`}>
                {isProfit ? '+' : ''}{netGain.toFixed(netGain !== 0 && Math.abs(netGain) < 0.00001 ? 5 : Math.abs(netGain) < 0.01 ? 4 : 3)} ETH
              </p>
              <p className="text-xs text-muted-foreground mt-1">
                (Started with {state.initialCredits.toFixed(state.initialCredits !== 0 && Math.abs(state.initialCredits) < 0.00001 ? 5 : Math.abs(state.initialCredits) < 0.01 ? 4 : 3)} ETH)
              </p>
            </div>
        </div>
